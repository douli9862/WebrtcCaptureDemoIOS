...
- (void)captureOutput:(AVCaptureOutput *)captureOutput
  ...
  _rotation = RTCVideoRotation_0;
    ...
    RTCCVPixelBuffer *rtcPixelBuffer = nil;//[[RTCCVPixelBuffer alloc] initWithPixelBuffer:pixelBuffer];
...
#endif
  NSLog(@"Capture session interrupted: %@", reasonString);
}
...
//#if defined(WEBRTC_IOS)
    _captureSession.sessionPreset = AVCaptureSessionPresetiFrame1280x720;//AVCaptureSessionPresetInputPriority;
...
- (void)setupVideoDataOutput {
  ...
  // device with the most efficient output format first. Find the first format that we support.
    NSSet<NSNumber *> *supportedPixelFormats = [NSSet setWithObjects:@(kCVPixelFormatType_420YpCbCr8BiPlanarFullRange),
                                                @(kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange),
                                                @(kCVPixelFormatType_32BGRA),
                                                @(kCVPixelFormatType_32ARGB),
                                                nil];//nil;//[RTCCVPixelBuffer supportedPixelFormats];
